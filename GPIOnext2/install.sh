...
file1=$SCRIPTPATH"/gpionext2.service"
cp $file1 /lib/systemd/system/
original='WorkingDirectory=/home/pi/gpionextmyconfig/gpionext2'
sed -i 's#'$original'#WorkingDirectory='$SCRIPTPATH'#g' /lib/systemd/system/gpionext2.service
systemctl enable gpionext2

# create Udev rule for SDL2 applications
# old udev rule -> UDEV='SUBSYSTEM=="input", ATTRS{name}=="GPIOnext2 Keyboard", ENV{ID_INPUT_KEYBOARD}="1"'
UDEV='KERNEL=="event*", ATTRS{idVendor}=="9999", ATTRS{idProduct}=="8888", MODE:="0644"'
echo $UDEV > /etc/udev/rules.d/10-gpionext2.rules
