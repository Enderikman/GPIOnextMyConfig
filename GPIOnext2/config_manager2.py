...
parser = argparse.ArgumentParser(description='GPIOnext2 Configuration Manager')
...
class ConfigurationManager:
	...
	def __init__( self, args ):
		...
		# Stop any running GPIOnext2 components
		subprocess.call(('systemctl', 'stop', 'gpionext2'))
		time.sleep(1) #give time to stop processes
		try:
			active = subprocess.check_output(['systemctl', 'is-active', 'gpionext2'])
		except:
			active = 'active'
		if 'active' in active:
			self.DEBUG('ERROR: systemctl stop gpionext2 has failed!')
			self.DEBUG('Please stop gpionext2 before running config!')
		else:
			self.DEBUG('gpionext2 service has been successfully stopped')
