...
parser = argparse.ArgumentParser(description='GPIOnext1 Configuration Manager')
...
class ConfigurationManager:
	...
	def __init__( self, args ):
		...
		try:
			active = subprocess.check_output(['systemctl', 'is-active', 'gpionext1'])
		except:
			active = 'active'
		if 'active' in active:
	...
	def getControllerType( self ):
		...
		if currentDevice == None:
			gpio.cleanup()
			print("Type: 'gpionext1 start' to run the daemon") 
			sys.exit(0)
