...
# old udev rule -> UDEV='SUBSYSTEM=="input", ATTRS{name}=="GPIOnext1 Keyboard", ENV{ID_INPUT_KEYBOARD}="1"'
UDEV='KERNEL=="event*", ATTRS{idVendor}=="9999", ATTRS{idProduct}=="8888", MODE:="0644"'
echo $UDEV > /etc/udev/rules.d/10-gpionext1.rules
...
#if yes, run gpionext1 config
if [[ ! -z $(echo ${USER_INPUT} | grep -i y) ]]; then
  sudo python3 $SCRIPTPATH/config_manager1.py
  echo "-------------> Setup Complete!"
fi
#Start GPIOnext1
systemctl daemon-reload
systemctl start gpionext1
